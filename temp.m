...
if isunix()
    ...
    %DATA_DIR = '~/Infomax/Tempotrons/PoC'; % directory with WAV files
    %DEST_DIR = DATA_DIR;
...
N_MAX = 10; %Inf; %NOTE: limits the files we process
...
for i_file=1:N_FILES
    ...
    try
        ...
        plot_raw_mel_flag = plot_flag; %0
        [S,F,T] = melSpectrogram_ex(audioIn,fs,plot_raw_mel_flag, ...
            'WindowLength',WL,...
            'OverlapLength',OLL, ...
            'FFTLength',FFTL, ...
            'NumBands',NumOfBands, ...
...
%         disp(F);
        ...
        if plot_flag
            ...
            imagesc(T, F, flipud(S)); hcb = colorbar;
            yticklabels(fliplr(yticks()));
            xlabel('Time (S)')
            ylabel('Frequency (Hz)')

            set(get(hcb,'label'),'string','Power (normalized)');
        ...
        for i_start=1:cycles:total-mod(total, cycles)
            ...
            if plot_flag && i_batch_item==5 %only on 5th
                ...
                fprintf('Band1 (Low freqs) #spks: %d\n', sum(sum(spk_train(end-30:end,:))))
                fprintf('Band16 (High freqs) #spks: %d\n', sum(sum(spk_train(1:31,:))))
            end
        ...
        %arr_total_spks(i_file) = sum(sum(spk_train(1:31,:))); %High freqs
        %arr_total_spks(i_file) = sum(sum(spk_train(end-30:end,:))); %Low freqs
    catch e
